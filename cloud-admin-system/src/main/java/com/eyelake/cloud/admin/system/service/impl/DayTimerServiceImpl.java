...
@Service("dayTimerService")
@ServiceTrace
public class DayTimerServiceImpl implements DayTimerService {

    private static final Logger LOGGER = LoggerFactory.getLogger(DayTimerServiceImpl.class);
...
//    }
    ...
    @Override
    public Result updateRtuWaringStatus(QueryRtuDto rtuDto) {
...
//        acsResponse.setCode("OK");
        ...
        if (acsResponse != null && REQUEST_SUCCESS_FLAG.equals(acsResponse.getCode())) {
            ...
            if (!CollectionUtils.isEmpty(acsResponse.getCardFlowInfos())) {
                ...
                String totalPackName = stringBuilder.toString();
                ...
                //更新网关预警表信息
                Result updateRtuWaringStatusResult = updateRtuWaringStatus(queryRtuDto);
        ...
        //请求失败处理逻辑，更新（插入）网关月流量，插入网关日流量表（设为0）
            ...
            LOGGER.error("调用QueryCardFlowInfoRequest接口失败处理逻辑开始");
            ...
            LOGGER.error("调用QueryCardFlowInfoRequest接口失败处理逻辑结束");
            result.setSuccess(false);
            return result;
        }
    ...
    private Date dateFormatToDay (Date date){
        ...
        Date newDate = null;
        try {
            newDate = sdf.parse(sdf.format(DateUtil.getDate()));

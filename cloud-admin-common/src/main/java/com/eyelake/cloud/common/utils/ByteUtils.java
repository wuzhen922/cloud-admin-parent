...
public class ByteUtils {
    ...
    public static long longChange(long mint) {
        ...
        mint = (long)(((mint) >> 56 & 0xFF) | ((mint) >> 48 & 0xFF00) | ((mint) >> 24 & 0xFF0000) | ((mint) >> 8 & 0xFF000000) | ((mint) << 8 & 0xFF00000000l) | ((mint) << 24 & 0xFF0000000000l)
    ...
    public static long byteArrayToLong(byte[] bytes) {
        return ((((long)bytes[0] & 0xff) << 24) | (((long)bytes[1] & 0xff) << 16) | (((long)bytes[2] & 0xff) << 8) | (((long)bytes[3] & 0xff) << 0));
    }
     ...
     * @return 是否通过校验
     */
    public static boolean checkSum(byte[] data, byte checkSum) {
        
        byte generatedCheckSum = 0x00;
     ...
     * @param begin 开始位置
     * @param end 结束位置
     * @param data 需要截取的byte数组
     * @return 截取的数组
     */
    public static byte[] subBytes(int begin, int end, byte[] data) {
        
        byte[] returnByte = new byte[end - begin];
        
        System.arraycopy(data, begin, returnByte, 0, end - begin);
